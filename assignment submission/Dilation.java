...
/**
 * The Dilation class presents a way to access a picture, dilate and save it.
 * 
 * @author Li Tang
 * @version 2018.01.18
 */
public class Dilation {
	public static void main(String [] args)
	{
		...
		//use dilation method. 
		dilation(img.height(),img.width(),img,img2);
	...
	/**
	 * dilate image and save as a new image. 
	 ...
	 */
	public static void dilation(int h,int w,Mat img,Mat img2) {
		for(int x=0;x<w;x++) {
			for(int y =0;y<h;y++) {
				double max=img.get(x,y)[0];
				//Get a point as a centre and find every pixel in the 5*5 square. 
				for (int i=x-2;i<=x+2;i++) {
					for(int j=y-2;j<=y+2;j++) {
						//compare every pixels in the square and find the biggest one.
						if (i>=0&&i<w&&j>=0&&j<h) {
				...
				//put the every maximum pixel in the new Mat.
				img2.put(x, y, max);
			}
		}	
	}
